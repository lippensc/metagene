...
nucleo <- as.numeric(c(0,0,1,1))
cyto <- as.numeric(c(1,1,0,0))
mydesign<-data.frame(Samples)
mydesign$cyto <- cyto
...
mg$plot(title = 'NDUFAB1 with normalized values under 10')
mg$produce_data_frame(avoid_gaps = TRUE,
                        bam_name = "c_al4_945MLM_demo_sorted",
                        gaps_threshold = 30)
mg$plot(title = 'NDUFAB1 without normalized values under 30')
```
...
This RNA-seq version of metagene can be used to detect differentially expressed 
genes. Indeed, as it is possible to evaluate differential expression on one 
gene across designs, this evaluation can be performed over all genes selected 
in our experiment.

See below an algorithm which perform this evaluation :

```{r differentially_expressed_genes}
regions <- 
c(system.file("extdata/ENCFF355RXX_DPM1.bed", package="metagene"),
system.file("extdata/ENCFF355RXX_NDUFAB1.bed", package="metagene"),
system.file("extdata/ENCFF355RXX_SLC25A5.bed", package="metagene"))
...
#remove values under 30
tabr <- avoid_gaps_update(tab, 
        bam_name = 'c_al4_945MLM_demo_sorted', 
        gaps_threshold = 30)
...
p_values <- c()
genes <- unique(tabr$region)

#For each gene
for (gene in genes){
    tab0 <- tabr[which(tabr$region == gene),]
    tab1 <- tab0[which(tab0$design == "cyto"),]
    tab2 <- tab0[which(tab0$design == "nucleo"),]
    ...
    p_values <- c(p_values, sum(ratio_intersect >= 
                                        permutation_results) / 
                                            length(permutation_results))
    print(paste('p-value for', gene, 'calculated'))
}
result <- data.frame(genes,p_values)
result$genes <- as.factor(genes) 
result$p_values <- as.numeric(p_values)
p_val_ord <- order(result$p_values)
result <- result[p_val_ord,]
result

#Multiple tests correction (for instance Benjamini, Hochberg) 
require(stats)
adjusted_result <- p.adjust(as.double(result$p_values), 
                    method = "BH", 
                    n = length(result$p_values))
adjusted_result
...
We can observe non significant p_values for DPM1 and SLC25A5, especially after 
correction for multiple tests. However, we only have two replicates by design 
and we can expect that with more replicates, these results could become 
significant regarding to the plot. The lines seem to be without rubans. Is 
because of number of replicates. Only two replicates lets the ruban be very 
close to the mean.
